...
def initializeArgs(option):
  ...
  if option == 'batteryvoltage':
    ...
    args['trained_model'] = '../models/batteryVoltage_local.h5'
    ...
    args['drop_columns'] = ['timestamp', 'time', 'diff_ms', 'temp', 'batt','engine_ON', 'speed_knots', 'lat', 'long']
    ...
    args['drop_columns'] = ['timestamp', 'time', 'diff_ms', 'temp', 'batt','batteryvoltage', 'speed_knots', 'lat', 'long']
  return args
...
def prepareData(args):
  ...
  if args['target_col'] == 'batteryvoltage':
    dataframe['target'] = dataframe[args['target_col']].apply(lambda x: int(x*10 - 12)) #FIXME
  ...
  if n_classes == 2:
    ...
    class_weight = None
  return feature_layer, n_classes, scaler, train_ds, val_ds, test_ds, class_weight

def fitModel(args, feature_layer, n_classes, train_ds, val_ds, class_weight):
  if args['target_col'] == 'batteryvoltage':
...
def evaluateHistory(args, history):
  ...
  for metric in [m.name for m in args['metrics']]:
    ...
    plt.plot(epochs, history.history['val_{}'.format(metric)], color='orange', label='Val')
    plt.xlabel('Epoch')
    plt.ylabel(metric)
    plt.legend()
...
def main(option):
  ...
  feature_layer, n_classes, scaler, train_ds, val_ds, test_ds, class_weight = prepareData(args)
  model, history = fitModel(args, feature_layer, n_classes, train_ds, val_ds, class_weight)
